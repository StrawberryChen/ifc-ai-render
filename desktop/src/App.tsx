...
export default function App() {
  ...
  useEffect(() => {
    if (!project) return;
    preloadModel(`${API}${project.source_model_url}?viewer=3`);
    preloadModel(`${API}${project.staged_model_url}?v=${revisions[0]?.number ?? 0}&viewer=3`);
  }, [project, revisions]);
  useEffect(() => {
    ...
    <section className="workspace">
      <div className="stage-panel">
        ...
        <div className="viewport">
          {project && <ModelViewer hidden={tab === "render"} modelUrl={`${API}${viewerTab === "source" ? project.source_model_url : project.staged_model_url}${viewerTab === "staged" ? `?v=${stagedVersion}&viewer=3` : "?viewer=3"}`} staged={viewerTab === "staged"} environmentUrl={viewerTab === "staged" && project.environment_url ? `${API}${project.environment_url}` : undefined} referenceRadius={viewerTab === "staged" ? sourceRadius : undefined} onReferenceRadius={viewerTab === "source" ? setSourceRadius : undefined} onViewChange={viewerTab === "staged" ? setCameraView : undefined} />}
